...
public class CategoryService {
    ...
    public CategoryDto create(CategoryDto dto) {
        if (dto.getEventId() == null) {
            throw new RuntimeException("Category must belong to an event (eventId required)");
        }
    ...
    private CategoryDto toDto(Category category) {
        ...
        return new CategoryDto(category.getId(), category.getName(), category.getTimeInitial(), category.getTimeFinal(), eventId);
    }

    private void validateCategoryTimesWithinEvent(Event event, Date start, Date end) {
        if (event == null) return; // no event to validate against
